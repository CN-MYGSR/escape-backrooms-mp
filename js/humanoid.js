/* ============ 怪物一：影者 —— 仿人类黑色实体 ============
 * 两米余高的漆黑人形，只有一双淡色的眼睛反光。
 * 行为：巡逻 → 听声/目击 → 追猎（BFS 寻路）→ 失联后前往最后目击点搜索。
 */
(function () {
  ...
  class Humanoid {
    constructor(scene, world) {
      ...
      this.stepPhase = 0;
      this.dormant = 0; // 开局宽限
      this.senseT = 16 + Math.random() * 14; // 隐约感知玩家方位的间隔
      this.speedBonus = 0;
      this.lastSeen = null;
      this.chaseLostT = 0;
    ...
    _buildMesh(scene) {
      ...
      g.scale.setScalar(1.14); // 总高约 2.5m，比人高
      this.group = g;
      scene.add(g);
    }
    ...
    canSee(player, range) {
      if (player.hidden) return false; // 幽灵（观战用假人）不被看见
    ...
    update(dt, player, t) {
      ...
      const sees = this.canSee(player, 30);
      const hears = player.sprinting && d < 20;

      // ---- 状态机 ----
      if (sees) {
        if (this.state !== ST.CHASE) {
          this.state = ST.CHASE;
          SFX.buzzFlicker(); // 追猎开始的电流爆音
        }
        ...
        if (this.chaseLostT > 4) {
          ...
          if (this.lastSeen) this.setPathTo(this.world.toCX(this.lastSeen.x), this.world.toCY(this.lastSeen.z));
        }
      } else if (hears && this.state === ST.PATROL) {
        this.state = ST.INVESTIGATE;
        this.setPathTo(player.cx, player.cy);
      }

      // 隐约感知：每隔一阵就朝玩家当前所在区域搜索（保证有遭遇）
      this.senseT -= dt;
      if (this.senseT <= 0) {
        this.senseT = 16 + Math.random() * 14;
      ...
      if (this.state === ST.CHASE) {
        ...
        if (sees && d < 11) {
          // 近距离直视：直线扑向玩家
          this._steer(dt, player.pos.x, player.pos.z, speed);
          this.path = null;
        } else {
        ...
        // 攻击（需近距离视线，防止隔墙拍人；观战模式下不攻击）
        if (d < 1.3 && this.attackCd <= 0 && !(window.Game && Game.spectate) &&
            this.world.los(this.pos.x, 1.9, this.pos.z, player.pos.x, player.pos.y, player.pos.z)) {
          this.attackCd = 1.7;
          this.attackAnim = 0.4;
          Game.attacker('黑色实体「影者」');
          player.hurt(34);
          player.knockback(this.pos.x, this.pos.z, 9);
          SFX.screech();
          UI.msg('它抓住了你！', 'danger', 1800);
        }
      ...
      } else { // PATROL：偏好玩家附近的房间游荡，制造遭遇
        if (!this.path || this._pathDone()) {
          if (this.idleT <= 0) {
            const r = this.world.roomNear(player.cx, player.cy, 3, 12);
    ...
    _steer(dt, tx, tz, speed) {
      ...
      // 脚步声（按移动距离累积，跑步约 3 步/秒）
      this.stepPhase += speed * dt * 0.62;
      if (this.stepPhase > 1) {
        this.stepPhase = 0;
        const pd = dist2D(this.pos.x, this.pos.z, window.Game ? Game.player.pos.x : 0, window.Game ? Game.player.pos.z : 0);
        SFX.monsterStep(clamp(1 - pd / 26, 0, 1) * 0.9);
      }
    }
