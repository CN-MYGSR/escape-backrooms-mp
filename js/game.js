...
(function () {
  ...
  Game.beginRound = function (seed, spawnIdx, roster) {
    ...
    // ---- B1 停车场（第二区域，全员确定性构建；进入前隐藏）----
    const garageSeed = (seed ^ 0x9e3779b9) >>> 0;
    ...
    Game.mimic.world = garage;
    const mCell = garage.openCell();
    Game.mimic.x = garage.toWX(mCell.cx); Game.mimic.z = garage.toWZ(mCell.cy);
    Game.rescuers = [0, 1].map(() => {
      const r = new Rescuer(garage.group);
      r.world = garage;
      const c = garage.openCell();
      r.x = garage.toWX(c.cx); r.z = garage.toWZ(c.cy);
      return r;
    });
    if (isHost() || Game.solo) {
    ...
    UI.show(null);
    UI.setObjective('准备…');
    UI.setBottles(0, Game.world.bottleTotal + garage.bottleTotal);
    UI.setSan(100);
    UI.setBattery(100);
  ...
  Game._step = function (dt, tNow) {
    ...
    if (!iAmShadow) {
      // 出口仅在 B1 停车场（zone 1）
      const ex = W.exit;
      if (Game.zone === 1 && !Game._flags.escaped && dist2D(P.pos.x, P.pos.z, ex.trigger.x, ex.trigger.z) < 1.4) {
    ...
    // ---- 电梯（区域切换，人类与影者均可双向乘坐）----
    Game.zoneCd = Math.max(0, (Game.zoneCd || 0) - dt);
    if (Game.zoneCd <= 0 && Game.worlds) {
      const lift = Game.zone === 0 ? W.exit : W.lift;
      if (lift && dist2D(P.pos.x, P.pos.z, lift.trigger.x, lift.trigger.z) < 1.35) {
        Game.elevT = (Game.elevT || 0) + dt;
        if (Game.elevT > 0.35 && !Game._elevMsg) {
          Game._elevMsg = true;
          UI.msg(Game.zone === 0 ? '电梯下行…' : '电梯上行…', null, 1800);
        }
        if (Game.elevT >= 1.1) _switchZone();
      } else {
        Game.elevT = 0; Game._elevMsg = false;
      }
    }

    // ---- 搜救队体力光环（停车场内靠近搜救队缓慢回体力）----
    if (Game.zone === 1 && Game.rescuers && !iAmShadow) {
      for (const r of Game.rescuers) {
        if (dist2D(P.pos.x, P.pos.z, r.x, r.z) < 5) {
          P.stamina = Math.min(100, P.stamina + 2.2 * dt);
          break;
        }
      }
    }
